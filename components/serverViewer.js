...
export default function ServerViewer(props) {
  ...
  return (
    <div className="serverViewer">
      ...
      <button onClick={() => REST.startServer(serverObject.serverId)}>
        Start
      </button>
      <button onClick={() => REST.stopServer(serverObject.serverId)}>
        Stop
      </button>
      <button onClick={() => REST.deleteServer(serverObject.serverId)}>
        Delete
      </button>
    </div>
  );
